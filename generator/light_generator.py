...
#
#     # def _verify_and_filter(self, candidate_list, quota, final_candidates, ctx):
#     #     # 【修改版】禁用了语义相似度、命名风格检查和启发式打分
#     #     # 仅保留：基础防呆 + AST 语法/作用域安全验证
#     #
#     #     added = 0
#     #     for cand in candidate_list:
#     #         if added >= quota:
...
#     #
#     #         # 1. 基础防呆：排除原名和保留关键字
#     #         if cand == ctx['target_name'] or cand in ctx['keywords']:
#     #             continue
#     #
#     #         # 2. 检查基本的作用域冲突（快速前置检查）
#     #         if not self.analyzer.can_rename_to(ctx['code_bytes'], ctx['target_name'], cand):
#     #             continue
#     #
#     #         # 3. 完整的 AST 验证（确保应用修改后语法树正常）
#     #         # 调用原本存在的验证单条候选词的方法
#     #         valid_cand = self._verify_ast_single(cand, ctx)
#     #
#     #         # 4. 如果 AST 验证通过，且未在结果集中，则加入
#     #         if valid_cand and valid_cand not in final_candidates:
#     #             final_candidates.append(valid_cand)
#     #             added += 1
#     #
...
#             variations = []
#
#             # 1. 【最高优先级】全掩码：无论多少个词，先尝试将整个变量替换为一个 MASK
#             variations.append({
#                 'expand_mode': 'full',
#                 'num_masks': 1,
#                 'masked_str': leading_us + mask_token
#             })
#
#             # 2. 【次优先级】其他细粒度掩码
#             if len(parts) == 1:
...
#                     for w in words:
#                         # 【新增逻辑】专门处理最高优先级的全掩码
#                         if var_info.get('expand_mode') in ('full', 'none'):
#                             meta["raw_mlm_cands"].append(f"{leading_us}{w}")
#
#                         elif var_info.get('expand_mode') == 'prefix':
...
#
#         cg_cfg = self.config.get('candidate_generation', {})
#         self.preserve_style = cg_cfg.get('preserve_style', True)
#         self.semantic_threshold = cg_cfg.get('lightweight', {}).get('semantic_threshold', 0.85)
#
#         stats_path = cg_cfg.get('naming_stats_path', 'naming_stats.json')
#         try:
#             from utils.scorer import StatisticalNamingScorer
...
#                 device)
#             with torch.no_grad():
#                 if hasattr(self.mlm_engine.model, 'roberta'):
#                     outputs = self.mlm_engine.model.roberta(**inputs)
#                 else:
#                     outputs = self.mlm_engine.model.base_model(**inputs)
#                 last_hidden = outputs.last_hidden_state
#
#             for b_idx in range(len(batch_texts)):
#                 p_text = batch_prefixes[b_idx]
#                 p_tokens = tokenizer.encode(p_text, add_special_tokens=False)
#                 pv_tokens = tokenizer.encode(p_text + batch_vars[b_idx], add_special_tokens=False)
#
#                 shared_len = sum(1 for pt, pvt in zip(p_tokens, pv_tokens) if pt == pvt)
#                 start_idx = min(shared_len + 1, 127)
#                 end_idx = min(max(start_idx + 1, len(pv_tokens) + 1), 128)
#
#                 pooled = last_hidden[b_idx, start_idx:end_idx, :].mean(dim=0)
#                 all_embeddings.append(pooled.to(torch.float32).cpu())
#
#         return torch.stack(all_embeddings)
#
#     def _verify_ast_single(self, code_bytes: bytes, identifiers: dict, target_name: str, cand: str) -> bool:
#         if not self.analyzer.can_rename_to(code_bytes, target_name, cand): return False
#         try:
#             CodeTransformer.validate_and_apply(code_bytes, identifiers, {target_name: cand}, analyzer=self.analyzer)
#             return True
#         except Exception:
#             return False
#
#     def generate_candidates(self, batch_tasks: List[Dict[str, Any]], top_k_mlm: int = 40, **kwargs) -> Dict[
#         str, List[str]]:
#         results = {task["target_name"]: [] for task in batch_tasks}
#         mask_token = self.mlm_engine.tokenizer.mask_token
#
...
#             target_info = slice_identifiers[target_name][best_occ_idx]
#
#             local_prefix, local_suffix = self._extract_local_context_ast(slice_code_bytes, target_info['start'],
#                                                                          target_info['end'], tree)
#
#             # 在精准的局部上下文中替换为 MASK
#             masked_code = local_prefix + mask_token + local_suffix
#             all_cropped_codes.append(masked_code)
#             valid_tasks.append(task)
#
#             task_metadata[task_idx] = {
#                 "target_name": target_name,
#                 "code_bytes": task["full_code_str"].encode("utf-8"),
#                 "identifiers": task["full_identifiers"],
#                 "local_prefix": local_prefix,
#                 "local_suffix": local_suffix,
#                 "original_style": self._detect_naming_style(target_name)
#             }
#
#         if not all_cropped_codes: return results
#
#         inputs = self.mlm_engine.tokenizer(
#             all_cropped_codes, return_tensors="pt", padding=True, truncation=True, max_length=512
#         ).to(self.mlm_engine.device)
#
#         mask_token_id = self.mlm_engine.tokenizer.mask_token_id
#         with torch.no_grad():
...
#             target_parts, _ = self._split_identifier(target_name)
#
#             mask_indices = (inputs.input_ids[i] == mask_token_id).nonzero(as_tuple=True)[0]
#             if len(mask_indices) == 0: continue
#
#             mask_logits = logits[i, mask_indices[0], :]
#             _, top_indices = torch.topk(mask_logits, top_k_mlm * 2, dim=-1)
#
#             raw_cands = []
#             for idx in top_indices:
#                 w = self.mlm_engine.tokenizer.decode([idx]).strip().replace('Ġ', '').replace('##', '')
#                 w = re.sub(r'[^a-zA-Z0-9_]', '', w)
#                 if w and w != target_name and w not in self.analyzer.keywords and w not in raw_cands:
#                     if self.preserve_style and not self._matches_style(meta["original_style"], w): continue
#                     raw_cands.append(w)
#
#             if not raw_cands: continue
#
#             orig_emb = self._get_variable_token_embeddings([meta["local_prefix"]], [target_name],
#                                                            [meta["local_suffix"]]).to(self.mlm_engine.device)
#             prefixes = [meta["local_prefix"]] * len(raw_cands)
#             suffixes = [meta["local_suffix"]] * len(raw_cands)
#             cand_embs = self._get_variable_token_embeddings(prefixes, raw_cands, suffixes).to(self.mlm_engine.device)
#
#             sims = F.cosine_similarity(orig_emb, cand_embs)
#             final_cands = []
#
#             for cand, sim in zip(raw_cands, sims):
#                 bonus = 0.0
#                 if self.scorer:
#                     cand_parts, _ = self._split_identifier(cand)
#                     bonus = self.scorer.calculate_heuristic_score(cand_parts, 'VARIABLE', target_parts, None)
#                 if bonus <= -100: continue
#
#                 final_score = sim.item() + bonus
#                 dynamic_threshold = self._get_dynamic_threshold(target_name, cand, self.semantic_threshold)
#
#                 if final_score >= dynamic_threshold:
#                     if self._verify_ast_single(meta["code_bytes"], meta["identifiers"], target_name, cand):
#                         final_cands.append(cand)
#
#             results[target_name] = final_cands
#
#         return results
...
class LightweightCandidateGenerator:
    ...
    @torch.no_grad()
    def _calculate_perplexity_batch(self, texts: List[str], batch_size: int = 4) -> List[float]:
        if not texts or not self.llm_client: return [0.0] * len(texts)
        tokenizer = getattr(self.llm_client, 'tokenizer', None)
        model = getattr(self.llm_client, 'model', None)
        if not tokenizer or not model: return [0.0] * len(texts)
        ...
        for i in range(0, len(texts), batch_size):
            batch_texts = texts[i:i + batch_size]
            inputs = tokenizer(batch_texts, return_tensors="pt", padding=True, truncation=True, max_length=1024).to(
                device)
            outputs = model(input_ids=inputs["input_ids"], attention_mask=inputs["attention_mask"])
            shift_logits = outputs.logits[..., :-1, :].contiguous()
            shift_labels = inputs["input_ids"][..., 1:].contiguous()
    ...
    # def _extract_local_context_ast(self, code_bytes: bytes, target_start: int, target_end: int, tree=None) -> tuple[
    #     str, str]:
    #     # [消融] 不再向上遍历 AST 寻找完整语句，直接暴力截断前后 500 字节
    #     prefix = code_bytes[:target_start].decode("utf-8", errors="replace")
    #     suffix = code_bytes[target_end:].decode("utf-8", errors="replace")
    #
    #     local_prefix = prefix[-500:] if len(prefix) > 500 else prefix
    #     local_suffix = suffix[:500] if len(suffix) > 500 else suffix
    #     return local_prefix, local_suffix
    #
    # def _find_best_context_occurrence(self, code_bytes: bytes, occurrences: List[dict], tree=None) -> int:
    #     # [消融] 不再对复杂的控制流块打分，直接返回该变量第一次出现的位置
    #     return 0
    ...
    def _decode_words(self, mask_logits, top_k, allow_underscore=False):
        _, top_indices = torch.topk(mask_logits, top_k, dim=-1)
        words = []
        for idx in top_indices:
            w = self.mlm_engine.tokenizer.decode([idx]).strip().replace('Ġ', '').replace('##', '')
    ...
    def _verify_ast_single(self, cand: str, ctx: dict) -> str | None:
        if not self.analyzer.can_rename_to(ctx['code_bytes'], ctx['target_name'], cand): return None
        try:
            from utils.ast_tools import CodeTransformer
            CodeTransformer.validate_and_apply(ctx['code_bytes'], ctx['identifiers'], {ctx['target_name']: cand},
                                               analyzer=self.analyzer)
    ...
    def _verify_and_filter(self, candidate_list, quota, final_candidates, ctx):
        ...
        for i in range(0, len(base_cands), CHUNK_SIZE):
            ...
            if base_threshold > 0:
                ...
                for cand, sim, bonus in zip(filtered_chunk, sims, heuristic_bonuses):
                    final_score = sim.item() + bonus
                    dynamic_threshold = self._get_dynamic_threshold(target_name, cand, base_threshold)
                    if final_score >= dynamic_threshold:
                        semantically_valid.append((cand, final_score))
            else:
                semantically_valid = [(cand, 1.0) for cand in filtered_chunk]
    ...
    def generate_candidates(self, batch_tasks: List[Dict[str, Any]], top_k_mlm: int = 40, top_n_keep: int = 20) -> Dict[
        ...
        for task_idx, task in enumerate(batch_tasks):
            ...
            # 🌟 核心修改：移除前缀、后缀、双孔掩码，统一强制使用【全部掩码】替换变量整体
            variations = [
        ...
        for t_idx, meta in task_metadata.items():
            ...
            ctx = {
                ...
                'ppl_max_abs': cg_cfg.get('ppl_max_abs', 50.0),
                'entity_type': meta["entity_type"],
                'return_type': next((u['return_type'] for u in meta["full_identifiers"].get(meta["target_name"], []) if
                                     u.get('return_type')), None),
            }
