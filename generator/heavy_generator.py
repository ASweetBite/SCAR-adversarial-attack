...
class HeavyWeightCandidateGenerator:
    ...
    #
    #     local_prefix = prefix[-5:] if len(prefix) > 500 else prefix
...
#         self.semantic_threshold = cg_cfg.get('heavyweight', {}).get('semantic_threshold', 0.85)
#
#         stats_path = cg_cfg.get('naming_stats_path', 'naming_stats.json')
#         try:
#             from utils.scorer import StatisticalNamingScorer
...
#
#             with torch.no_grad():
#                 if hasattr(self.embedder.model, 'roberta'):
#                     outputs = self.embedder.model.roberta(**inputs)
#                 else:
#                     outputs = self.embedder.model.base_model(**inputs)
...
#             ex_short = "'shm_info', 'mem_data', 'idx'"
#
#         leading_us_rule = "\n- PRESERVE PREFIX: The original name starts with '_'. ALL your suggestions MUST start with '_'." if target_name.startswith(
#             '_') else ""
#
#         strategy_instruction = f"""[Strategy: Short & Concise]
# - MAX WORDS: {n_parts + 2} words per name.
# - EXAMPLES: {ex_short}"""
#
#         return f"""You are an expert C/C++ developer. Suggest exactly {top_n} alternative names for `{target_name}`.
#
...
#             tree = parser.parse(slice_code_bytes)
#             slice_identifiers = self.analyzer.extract_identifiers(slice_code_bytes)
#             if target_name not in slice_identifiers: continue
#
#             best_occ_idx = self._find_best_context_occurrence(slice_code_bytes, slice_identifiers[target_name], tree)
#             target_info = slice_identifiers[target_name][best_occ_idx]
#
...
#                 "entity_type": entity_type, "original_style": original_style,
#                 "full_code_bytes": task["full_code_str"].encode("utf-8"),
#                 "full_identifiers": task["full_identifiers"],
#                 "local_prefix": local_prefix, "local_suffix": local_suffix
#             }
#
#             # 依然发送折叠切片代码，因为 LLM 理解上下文比 MLM 强得多，但为了防止 OOM，做了限制
#             prompt_code = slice_code_str
#             if len(prompt_code) > 2500: prompt_code = prompt_code[:2500]
#
#             prompt = self._build_llm_prompt(prompt_code, target_name, original_style, int(target_quota * 1.5),
#                                             entity_type, len(parts))
#             llm_prompts.append(prompt)
#
#         if not llm_prompts: return results
#
...
#
#                     # 第一道防线：关键字和样式
#                     w = re.sub(r'[^a-zA-Z0-9_]', '', clean_cand)
#                     if w and w != target_name and w not in self.analyzer.keywords and w not in valid_cands:
#                         if self.preserve_style and not self._matches_style(meta["original_style"], w): continue
#                         valid_cands.append(w)
#
#             if not valid_cands: continue
#
#             # 余弦相似度过滤
#             orig_emb = self._get_variable_token_embeddings([meta["local_prefix"]], [target_name],
#                                                            [meta["local_suffix"]]).to(self.embedder.device)
#             prefixes = [meta["local_prefix"]] * len(valid_cands)
#             suffixes = [meta["local_suffix"]] * len(valid_cands)
#             cand_embs = self._get_variable_token_embeddings(prefixes, valid_cands, suffixes).to(self.embedder.device)
#
#             sims = F.cosine_similarity(orig_emb, cand_embs)
#             final_cands = []
#
#             for cand, sim in zip(valid_cands, sims):
#                 bonus = 0.0
#                 if self.scorer:
#                     cand_parts, _ = self._split_identifier(cand)
#                     bonus = self.scorer.calculate_heuristic_score(cand_parts, 'VARIABLE', meta["parts"], None)
#
#                 if bonus <= -100: continue
#                 if sim.item() + bonus >= self.semantic_threshold:
#                     if self._verify_ast_single(meta["full_code_bytes"], meta["full_identifiers"], target_name, cand):
#                         final_cands.append(cand)
#
#             results[target_name] = final_cands[:target_quota]
#
#         return results