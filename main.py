...
def main(args, config):
    ...
    evaluator = SCARAttacker(
        ...
        get_all_vars_fn=get_all_identifiers_fn,
        mlm_gen=lightweight_generator,
        llm_gen=heavyweight_generator,  # 重新启用 LLM
        rename_fn=rename_fn,
        mode=args.mode,
        config=config
    )
    ...
    print(f"\n[*] Loading dataset in {args.mode} mode...")
    dataset = loader.load_parquet_dataset(
        filepath=config['run_params']['dataset'],
        mode=args.mode,
        max_samples=config['run_params']['samples'],
        label_map_path=config['run_params'].get('label_map'),
        random_seed=config['global'].get('random_seed', 42)
    )
